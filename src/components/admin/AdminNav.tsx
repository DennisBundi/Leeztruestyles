...
export default function AdminNav({ userRole: propUserRole, employee: propEmployee }: AdminNavProps = {}) {
  ...
  return (
    <>
      {/* Top Bar */}
      <nav className="bg-[#4a0026]/90 backdrop-blur-md border-b border-white/10 sticky top-0 z-40">
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-4">
              ...
              <Link href="/dashboard" className="flex items-center gap-2">
                ...
                />
                <span className="text-sm font-semibold text-white/70 hidden sm:inline">Admin</span>
              </Link>
            </div>
            <div className="flex items-center gap-4">
              <Link
                href="/"
                target="_blank"
                className="text-sm text-white/60 hover:text-white transition-colors hidden sm:flex items-center gap-1"
              >
      ...
      {/* Sidebar */}
      <aside className={`fixed left-0 top-16 h-[calc(100vh-4rem)] bg-[#3d001e]/95 backdrop-blur-md border-r border-white/10 transition-all duration-300 z-30 ${sidebarOpen ? 'w-20 lg:w-64' : 'w-20 lg:w-20'} translate-x-0`}>
        <nav className="h-full py-6 px-4 overflow-y-auto flex flex-col">
          ...
          {user && (
            <div className={`border-t border-white/10 pt-4 mt-4 hidden ${sidebarOpen ? 'lg:block' : ''}`}>
              ...
              <button
                ...
                className="w-full flex items-center justify-center lg:justify-start gap-2 px-2 lg:px-4 py-2.5 bg-white/10 hover:bg-white/15 text-white/70 rounded-lg transition-colors font-medium text-sm"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
