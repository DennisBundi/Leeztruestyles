...
export default function ProductForm({ categories, product, onSuccess, onClose }: ProductFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!hasDatabase) {
        ...
        alert(product ? 'Product updated successfully! (Preview Mode)' : 'Product created successfully! (Preview Mode)');
      ...
      const response = await fetch('/api/products', {
        ...
        body: JSON.stringify({
          ...
          images,
          flash_sale_start: formData.is_flash_sale && formData.flash_sale_start ? new Date(formData.flash_sale_start).toISOString() : null,
          flash_sale_end: formData.is_flash_sale && formData.flash_sale_end ? new Date(formData.flash_sale_end).toISOString() : null,
        }),
      });
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  {formData.sale_price && formData.price && (
                    <p className="text-xs text-gray-500 mt-1">
                      Discount: {Math.round((1 - parseFloat(formData.sale_price.toString()) / parseFloat(formData.price.toString())) * 100)}%
                    </p>
                  )}
                </div>
                ...
                {/* Flash Sale */}
                <div className="md:col-span-2">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4 mt-4">Flash Sale</h3>
                </div>
                ...
                <div className="md:col-span-2">
                  ...
                  <p className="text-xs text-gray-500 mt-3">
                    Upload multiple images. The first image will be used as the main product image. You can reorder images by using the arrow buttons.
                  </p>
                </div>
